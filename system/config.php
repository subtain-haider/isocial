...
*/
    ...
    if (!file_exists($K->INCPATH . 'preconfig.php') ) {
            header("Location: ".sprintf("%s://%s%s", isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] != 'off' ? 'https' : 'http', $_SERVER['SERVER_NAME'], $_SERVER['REQUEST_URI'])."install"); 
            exit();
	}
	...
	require_once($K->INCPATH.'preconfig.php');
    ...
    $K->FILE_SIZE_PHOTO_PRODUCTS = 5 * 1024 * 1024; // 5 MB;
    ...
    $K->STORAGE_URL_OGS = $K->STORAGE_URL.'ogs/';
    $K->STORAGE_DIR_OGS = $K->STORAGE_DIR.'ogs/';
    
    $K->MAIL_SSL = FALSE; // FALSE: with tls;   TRUE: with ssl (port 465) 
    ...
    $K->STORAGE_URL_MISC = $K->STORAGE_URL.'misc/';
    $K->STORAGE_DIR_MISC = $K->STORAGE_DIR.'misc/';
