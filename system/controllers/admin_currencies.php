...
*/
    ...
    $items = $this->db2->fetch_all("SELECT * FROM currencies ORDER BY name ASC");
    ...
    foreach ($items as $oneitem) {
        ...
        $D->html_items .= $this->load_template('ones/one-currency.php', FALSE);
    }
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            ...
            $for_load = 'min/admin-currencies.php';

		} else {

            $for_load = 'max/admin-currencies.php';

		}

        $D->titlePhantom = $this->lang('admin_currencies_title_page');
		...
		$D->page_title = $this->lang('admin_currencies_title_page');    	

        $D->file_in_template = 'max/admin-currencies.php';
