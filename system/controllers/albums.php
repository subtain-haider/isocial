...
*/
    ...
    $res = $this->db2->query("SELECT albums.*, posts.code as pcode FROM albums, posts WHERE idcreator=".$this->user->info->iduser." AND albums.idpost=posts.idpost ORDER BY idalbum DESC LIMIT 0, ".($K->ITEMS_PER_PAGE + 1));
    ...
    while ($obj = $this->db2->fetch_object($res)) {
        ...
        $D->album->post_code = stripslashes($D->album->pcode);
        $D->album->the_username = stripslashes($this->user->info->user_username);

        $D->album_last = FALSE;
        if ($total_items < $count_regs + 2) $D->album_last = TRUE;

        $D->the_list_items .= $this->load_template('ones/one-album.php', FALSE);
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            $for_load = 'min/albums.php';
		} else {
            $for_load = 'max/albums.php';
		}

        $D->titlePhantom = $this->lang('dashboard_albums_title_page');
        ...
        $D->page_title = $this->lang('dashboard_albums_title_page');

        $D->file_in_template = 'max/albums.php';
