...
*/
    ...
    if ($D->show_activity) {
        ...
        $res = $this->db2->query("SELECT * FROM posts WHERE (posted_in=2 AND id_wall=".$D->idgroup.") ORDER BY idpost DESC LIMIT 0, ".($K->ACTIVITIES_PER_PAGE + 1));
	...
	if ($D->isPhantom) {
        ...
        if ($D->layout_size == 'min') {
            $for_load = 'min/group.php';
        } else {
            $for_load = 'max/group.php';
        }
        ...
        if ($D->_IS_LOGGED) {
            ...
            $D->id_container = 'dashboard';
            $this->load_extract_controller('_dashboard-menu-left');
            $D->file_in_template = 'max/group.php';            
