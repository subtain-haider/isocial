...
*/
	...
	$D->totalitems = $this->db2->fetch_field('SELECT count(iduser) FROM users WHERE is_admin=1 '.$D->qsql);
	$D->start = ($D->pageCurrent-1) * $D->USER_PER_PAGE;

    /**** Pagination ****/

    $D->url_list = $K->SITE_URL.'admin/users/admins/';
    ...
    $items = $this->db2->fetch_all("SELECT * FROM users WHERE is_Admin=1 ".$D->qsql." ORDER BY user_username ASC LIMIT ".$D->start.",".$D->USER_PER_PAGE);
    ...
    $D->id_menu = 'opt_adm_users_admins';
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            ...
            $for_load = 'min/admin-users-admins.php';

		} else {

            $for_load = 'max/admin-users-admins.php';

		}

        $D->titlePhantom = $this->lang('admin_users_admin_title_page');
		...
		$D->page_title = $this->lang('admin_users_admin_title_page');    	

        $D->file_in_template = 'max/admin-users-admins.php';
