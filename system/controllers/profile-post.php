...
*/
    ...
    if ($D->post_is_showeable) {
        if ($obj_post->typepost == 12) {
            $product = $this->db2->fetch("SELECT * FROM products WHERE idpost=".$obj_post->idpost." LIMIT 1");
            if ($product) {
                                
                $D->OG_show = TRUE;
                $D->OG_type = 'product.item';
                $D->OG_title = stripslashes($product->name);
                $D->OG_description = stripslashes($product->description);
                $D->OG_url = $K->SITE_URL.$D->thepost_username.'/post/'.$D->thepost_code;;
                ...
                if ($photos_prod) {
                    foreach ($photos_prod as $onephoto) {
                        $photos_produc[] = $onephoto->photo;
                    }
                }
                $D->OG_image = $K->STORAGE_URL_PRODUCTS . 'min2/' . $photos_produc[0];
