...
*/
    ...
    if (strlen($D->codeads) != 11) $this->globalRedirect($K->SITE_URL.'admin/ads/profile');

    $theads = $this->db2->fetch("SELECT * FROM advertising_basic WHERE idslot=2 AND code='".$D->codeads."' LIMIT 1");

    if (!$theads) $this->globalRedirect($K->SITE_URL.'admin/ads/profile');
    ...
    $D->text_type = $this->lang('admin_ads_profile_edit_type_ads_image');
    if ($D->type_ads == 2) $D->text_type = $this->lang('admin_ads_profile_edit_type_ads_html');
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            ...
            $for_load = 'min/admin-ads-profile-edit.php';

		} else {

            $for_load = 'max/admin-ads-profile-edit.php';

		}

        $D->titlePhantom = $this->lang('admin_ads_profile_edit_title_page');
		...
		$D->page_title = $this->lang('admin_ads_profile_edit_title_page');    	

        $D->file_in_template = 'max/admin-ads-profile-edit.php';
