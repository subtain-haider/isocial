...
*/
    ...
    if (!$K->WITH_LIBRARY) $this->globalRedirect('dashboard');
	...
	if ($D->isPhantom) {
        ...
        if ($D->layout_size == 'min') {
            $for_load = 'min/articles-publish.php';
		} else {
            $for_load = 'max/articles-publish.php';
		}

        $D->titlePhantom = $this->lang('dashboard_articles_publish_title_page');
        ...
        $D->page_title = $this->lang('dashboard_articles_publish_title_page');

        $D->file_in_template = 'max/articles-publish.php';
