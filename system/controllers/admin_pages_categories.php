...
*/
    ...
    $items = $this->db2->fetch_all("SELECT * FROM pages_cat WHERE idfather=0 ORDER BY name ASC");
    ...
    foreach ($items as $oneitem) {
        ...
        $D->html_items .= $this->load_template('ones/one-category-page.php', FALSE);
    }
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            ...
            $for_load = 'min/admin-pages-categories.php';

		} else {

            $for_load = 'max/admin-pages-categories.php';

		}

        $D->titlePhantom = $this->lang('admin_pages_categories_title_page');
		...
		$D->page_title = $this->lang('admin_pages_categories_title_page');    	

        $D->file_in_template = 'max/admin-pages-categories.php';
