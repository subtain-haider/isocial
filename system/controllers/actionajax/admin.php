...
*/
    ...
    if ($ajax_action == 'updateyourbrand') {
        
        $dochange = isset($_POST['chgi']) ? (trim($_POST['chgi'])) : '';
        $dochange = $the_sanitaze->str_nohtml($dochange);

        $idform = isset($_POST['idform']) ? (trim($_POST['idform'])) : '';
        $idform = $the_sanitaze->str_nohtml($idform);

        if (!$error && !empty($idform)) {
            if ($dochange == '1') {
                $the_photo = $_FILES['imagenfile'.$idform];
                if (!$error && empty($the_photo)) { $error = TRUE; $txterror .= 'Error. '; }
            }
        } else { $error = TRUE; $txterror .= 'Error. '; }

    }

    if ($ajax_action == 'updatemodules') {

        $modpages = isset($_POST['mdp']) ? (trim($_POST['mdp'])) : -1;
        $modpages = $the_sanitaze->int($modpages);

        $modgroups = isset($_POST['mdgr']) ? (trim($_POST['mdgr'])) : -1;
        $modgroups = $the_sanitaze->int($modgroups);

        $modevents = isset($_POST['mde']) ? (trim($_POST['mde'])) : -1;
        $modevents = $the_sanitaze->int($modevents);

        $modmarket = isset($_POST['mdm']) ? (trim($_POST['mdm'])) : -1;
        $modmarket = $the_sanitaze->int($modmarket);

        $modlibrary = isset($_POST['mdl']) ? (trim($_POST['mdl'])) : -1;
        $modlibrary = $the_sanitaze->int($modlibrary);

        $modgames = isset($_POST['mdgm']) ? (trim($_POST['mdgm'])) : -1;
        $modgames = $the_sanitaze->int($modgames);

        if (!$error && $modpages == -1) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }
        if (!$error && $modgroups == -1) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }
        if (!$error && $modevents == -1) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }
        if (!$error && $modmarket == -1) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }
        if (!$error && $modlibrary == -1) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }
        if (!$error && $modgames == -1) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }

    }

    if ($ajax_action == 'updategdpr') {

        $mgdpr = isset($_POST['mgdpr']) ? (trim($_POST['mgdpr'])) : -1;
        $mgdpr = $the_sanitaze->int($mgdpr);

        if (!$error && $mgdpr == -1) { $error = TRUE; $txterror .= 'Error. '; }
        
    }

    if ($ajax_action == 'updatehashtagm') {

        $hash_status = isset($_POST['hss']) ? (trim($_POST['hss'])) : -1;
        $hash_status = $the_sanitaze->int($hash_status);

        $hash_interval = isset($_POST['hsi']) ? (trim($_POST['hsi'])) : -1;
        $hash_interval = $the_sanitaze->int($hash_interval);

        $hash_limit = isset($_POST['hsl']) ? (trim($_POST['hsl'])) : 0;
        $hash_limit = $the_sanitaze->int($hash_limit);

        if (!$error && $hash_status < 0) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }
        if (!$error && $hash_interval  < 0) { $error = TRUE; $txterror .= $page->lang('admin_txt_must_choose_option'); }
        if (!$error && $hash_limit  <= 0) { $error = TRUE; $txterror .= $page->lang('admin_general_block_hashtag_error_limit'); }
...
/******/
        ...
        if ($ajax_action == 'updateyourbrand') {
            ...
            if ($dochange == '1') {
                ...
                if ($the_photo['name']) { 
                    ...
                    $the_pholder_misc = $K->STORAGE_DIR_MISC;

                    $fname = getCode(5,1).'-'.getCode(5,1).'-'.getCode(5,1).$file_extension;
                    move_uploaded_file($the_photo['tmp_name'], $the_pholder_misc.$fname);
                    
                    switch($idform) {
                        case '01':
                            $page->db2->query("UPDATE settings SET value='".$fname."' WHERE word='LOGOTIPO_OUT' LIMIT 1");
                            $file_prev = $K->LOGOTIPO_OUT;
                            break;

                        case '02':
                            $page->db2->query("UPDATE settings SET value='".$fname."' WHERE word='LOGOTIPO_IN' LIMIT 1");
                            $file_prev = $K->LOGOTIPO_IN;
                            break;
                            
                        case '03':
                            $page->db2->query("UPDATE settings SET value='".$fname."' WHERE word='LOGOTIPO_RESP_OUT' LIMIT 1");
                            $file_prev = $K->LOGOTIPO_RESP_OUT;
                            break;
                            
                        case '04':
                            $page->db2->query("UPDATE settings SET value='".$fname."' WHERE word='LOGOTIPO_RESP_IN' LIMIT 1");
                            $file_prev = $K->LOGOTIPO_RESP_IN;
                            break;
                            
                        case '05':
                            $page->db2->query("UPDATE settings SET value='".$fname."' WHERE word='IMG_WELCOME_DASH' LIMIT 1");
                            $file_prev = $K->IMG_WELCOME_DASH;
                            break;
                            
                        case '06':
                            $page->db2->query("UPDATE settings SET value='".$fname."' WHERE word='IMG_FAVICON' LIMIT 1");
                            $file_prev = $K->IMG_WELCOME_DASH;
                            break;
                    }
                    
                    if ($file_prev) {
                        $the_file = $the_pholder_misc.$file_prev;
                        if (file_exists($the_file)) @unlink($the_file);
                    }
                    
                    $json_result = array('msg_ok' => $page->lang('admin_txt_has_updated'));
                    echo(json_encode($json_result));
                    return;
        ...
        if ($ajax_action == 'updatemodules') {
            ...
            $page->db2->query("UPDATE settings SET value='".$modpages."' WHERE word='WITH_PAGES' LIMIT 1");
            $page->db2->query("UPDATE settings SET value='".$modgroups."' WHERE word='WITH_GROUPS' LIMIT 1");
            $page->db2->query("UPDATE settings SET value='".$modevents."' WHERE word='WITH_EVENTS' LIMIT 1");
            $page->db2->query("UPDATE settings SET value='".$modmarket."' WHERE word='WITH_MARKETPLACE' LIMIT 1");
            $page->db2->query("UPDATE settings SET value='".$modlibrary."' WHERE word='WITH_LIBRARY' LIMIT 1");
            $page->db2->query("UPDATE settings SET value='".$modgames."' WHERE word='WITH_GAMES' LIMIT 1");
        ...
        if ($ajax_action == 'updategdpr') {
            ...
            $page->db2->query("UPDATE settings SET value='".$mgdpr."' WHERE word='WITH_GDPR' LIMIT 1");
        ...
        if ($ajax_action == 'updatehashtagm') {
            ...
            $page->db2->query("UPDATE settings SET value='".$hash_status."' WHERE word='WITH_HASHTAGS' LIMIT 1");
            $page->db2->query("UPDATE settings SET value='".$hash_interval."' WHERE word='WITH_HASHTAGS_INTERVAL' LIMIT 1");
            $page->db2->query("UPDATE settings SET value='".$hash_limit."' WHERE word='WITH_HASHTAGS_LIMIT' LIMIT 1");
