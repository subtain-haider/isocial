...
*/
    ...
    if ($D->show_activity) {
        ...
        $all_items = $this->db2->fetch_all("SELECT users.* FROM users, events_actions WHERE users.iduser=events_actions.iduser AND idevent=".$D->idevent." AND type_action=1 AND active=1 ORDER BY users.firstname ASC");
	...
	if ($D->isPhantom) {
        ...
        if ($D->layout_size == 'min') {
            $for_load = 'min/event-interested.php';
        } else {
            $for_load = 'max/event-interested.php';
        }
        ...
        if ($D->_IS_LOGGED) {
            ...
            $D->id_container = 'dashboard';
            $this->load_extract_controller('_dashboard-menu-left');
            $D->file_in_template = 'max/event-interested.php';
