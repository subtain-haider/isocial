...
*/
    ...
    $items = $this->db2->fetch_all("SELECT * FROM games WHERE status=1 ORDER BY name ASC");

    $D->html_items = '';
    $D->numgames = count($items);
    foreach ($items as $oneitem) {
        $D->one = $oneitem;
        $D->one->name = stripslashes($oneitem->name);
        $D->html_items .= $this->load_template('ones/one-game.php', FALSE);
    }
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            ...
            $for_load = 'min/admin-games.php';

		} else {

            $for_load = 'max/admin-games.php';

		}

        $D->titlePhantom = $this->lang('admin_games_title_page');
		...
		$D->page_title = $this->lang('admin_games_title_page');    	

        $D->file_in_template = 'max/admin-games.php';
