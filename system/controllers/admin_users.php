...
*/
	...
	$D->totalitems = $this->db2->fetch_field('SELECT count(iduser) FROM users '.$D->qsql);

	$D->start = ($D->pageCurrent-1) * $D->USER_PER_PAGE;

    /**** Pagination ****/

    $D->url_list = $K->SITE_URL.'admin/users/';
    ...
    $items = $this->db2->fetch_all("SELECT * FROM users ".$D->qsql." ORDER BY user_username ASC LIMIT ".$D->start.",".$D->USER_PER_PAGE);
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            ...
            $for_load = 'min/admin-users.php';

		} else {

            $for_load = 'max/admin-users.php';

		}

        $D->titlePhantom = $this->lang('admin_users_title_page');
		...
		$D->page_title = $this->lang('admin_users_title_page');    	

        $D->file_in_template = 'max/admin-users.php';
