...
*/
	...
	$D->totalitems = $this->db2->fetch_field('SELECT count(idarticle) FROM articles '.$D->qsql);
	$D->start = ($D->pageCurrent-1) * $D->PAGES_PER_PAGE;

    /**** Pagination ****/

    $D->url_list = $K->SITE_URL.'admin/articles/';
    ...
    $items = $this->db2->fetch_all("SELECT idarticle, articles.code, title, users.user_username, users.firstname, users.lastname, photo FROM articles, users WHERE iduser=idwriter ".$D->qsql." ORDER BY idarticle DESC LIMIT ".$D->start.",".$D->PAGES_PER_PAGE);
    ...
    foreach ($items as $oneitem) {
        ...
        $D->one->photo = $K->STORAGE_URL_ARTICLES.'min2/'.$D->one->photo;
        ...
        $D->one->url = $K->SITE_URL.'article/'.$D->one->code;
        $D->html_items .= $this->load_template('ones/one-article-list.php', FALSE);
	...
	if ($D->isPhantom) {
		...
		if ($D->layout_size == 'min') {
            ...
            $for_load = 'min/admin-articles.php';

		} else {

            $for_load = 'max/admin-articles.php';

		}

        $D->titlePhantom = $this->lang('admin_articles_title_page');
		...
		$D->page_title = $this->lang('admin_articles_title_page');    	

        $D->file_in_template = 'max/admin-articles.php';
