...
*/
    ...
    $res = $this->db2->query("SELECT * FROM posts WHERE (typepost=2 AND idwriter=".$D->idpage." AND type_writer=1) OR (typepost=2 AND posted_in=1 AND id_wall=".$D->idpage.") ORDER BY idpost DESC LIMIT 0, ".($K->ACTIVITIES_PER_PAGE + 1));
	...
	if ($D->isPhantom) {
        ...
        if ($D->layout_size == 'min') {
            $for_load = 'min/page-videos.php';
        } else {
            $for_load = 'max/page-videos.php';
        }
        ...
        if ($D->_IS_LOGGED) {
            ...
            $D->id_container = 'dashboard';
            $this->load_extract_controller('_dashboard-menu-left');
            $D->file_in_template = 'max/page-videos.php';            
