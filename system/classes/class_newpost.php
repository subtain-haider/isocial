...
class newpost
{
    ...
    public function setMessage($message)
    {
        ...
        $txt_plain = htmlentities($message, ENT_QUOTES, 'utf-8');

        $pattern = '/(\s|^)((#|\x{ff03}){1}([0-9_\p{L}&;]*[_\p{L}&;][0-9_\p{L}&;]*))/u';
        $txt_plain = preg_replace_callback($pattern, function($matches) use(&$thehashtags) {

            $tg = html_entity_decode($matches[4], ENT_QUOTES);
