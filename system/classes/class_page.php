...
class page
{
    ...
    private function _parse_input()
    {
        ...
        if (strtolower($request[0]) == 'hashtag') {
            $numblocks = count($request_hashtag);
            if ($numblocks > 1) {
                $request[1] = (rawurldecode($request_hashtag[1]));
            }            
        }
        ...
        if ( file_exists($this->controllers.$first.'.php') ) {
            ...
            if ($first == 'info') {
                ...
                $numblocks = count($request);
                if ($numblocks > 1) {
                    $this->params->hashtag = $request[1]; //strtolower($request[1]);
                }
                $this->request[] = $first;
