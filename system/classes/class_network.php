...
class network
{
    ...
    public function getUserAleat($total, $exclude=0, $privacy=-1)
    {
        ...
        $condition = '';
        if ($user->is_logged) {
            $friends = $this->db2->fetch_all('SELECT iduser FROM friends, users WHERE active=1 AND accepted_date<>0 AND ((friend1=iduser AND friend2='.$user->id.') OR (friend1='.$user->id.' AND friend2=iduser))');
    ...
    public function getTrendingHashtags() {
        ...
        if ($K->WITH_HASHTAGS_INTERVAL == 0) $time_consider = 24 * $one_hour;
        if ($K->WITH_HASHTAGS_INTERVAL == 1) $time_consider = 7 * (24 * $one_hour);
        if ($K->WITH_HASHTAGS_INTERVAL == 2) $time_consider = 30 * (24 * $one_hour);

        $thetime = time();
        $time_consider = $thetime - $time_consider;
        
        $the_hashtags = $this->db2->fetch_all("SELECT *, COUNT(idhashtag) as frequency FROM hashtags WHERE thedate>'".$time_consider."' AND thedate<='".$thetime."' GROUP BY hashtag ORDER BY frequency DESC LIMIT ".$K->WITH_HASHTAGS_LIMIT);

        if (!$the_hashtags) return FALSE;
