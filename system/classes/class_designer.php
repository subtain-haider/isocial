...
class designer
{
    ...
    public function createOptionMenuTop($item)
    {
        ...
        $the_option = '<div id="'.$item['id_option'].'" class="ico-top '.(isset($item['nomargin']) && $item['nomargin'] ? 'nomarginleft' : '').' luminity"><span title="'.$item['tag_title'].'">';
    ...
    public function loadLogo()
    {
        global $K, $D;
        $the_logo = '';
        $the_logo .= '<a href="'. $K->SITE_URL.($D->_IS_LOGGED ? 'dashboard' : '') .'" title="'. htmlspecialchars($K->SITE_TITLE) .'" '.($D->_IS_LOGGED ? 'rel="phantom-max" target="dashboard-main-area"' : '').'>';        
        $the_logo .= '<div id="logo"><span id="logo-normal"><img src="'. getImageMisc($D->_IS_LOGGED ? $K->LOGOTIPO_IN : $K->LOGOTIPO_OUT) .'"></span><span id="logo-mini"><img src="'. getImageMisc($D->_IS_LOGGED ? $K->LOGOTIPO_RESP_IN : $K->LOGOTIPO_RESP_OUT) .'"></span></div>';
        $the_logo .= '</a>';
        return $the_logo;
    }
    ...
    {
        global $K;
        $the_favicon = '    <link href="'. getImageMisc($K->IMG_FAVICON) .'" type="image/png" rel="shortcut icon" />'."\n";
        return $the_favicon;
    }
