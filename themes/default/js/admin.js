...
function updateGDPR_Ok(response) {
...
function updateGDPR_Error(response) {
   "use strict";
    openandclose(paramsArray[0], msg_error_conection, 1700);
    setTimeout(function() { $(paramsArray[2]).removeAttr('disabled'); }, 2500);
}

function updateGDPR(diverror, divok, bsubmit) {

   "use strict";
    $(bsubmit).attr('disabled','true');
    
    var modgdpr = validationInput('zeroandpositive', '#modgdpr', diverror, txt_error_option, bsubmit, true);
    if (!modgdpr) return;
    ...
    invoke(params, updateGDPR_Ok, updateGDPR_Error);

}

/******************************************************/

function updateHashtagM_Ok(response) {
   switch (response.status) {
        case 'ERROR':
            openandclose(paramsArray[0], response.message, 1700);
            setTimeout(function() { $(paramsArray[2]).removeAttr('disabled'); }, 2500);
...
function updateHashtagM_Error(response) {
    openandclose(paramsArray[0], msg_error_conection, 1700);
    setTimeout(function() { $(paramsArray[2]).removeAttr('disabled'); }, 2500);
}

function updateHashtagM(diverror, divok, bsubmit) {
    ...
    var params = {
            ...
            withFile: false,
            module: 'admin',
            action: 'updatehashtagm',
            cancelable: 0,
            data: data
    };

    invoke(params, updateHashtagM_Ok, updateHashtagM_Error);
